...
void CPyramid::GetRect(CMatrix& Vert, CRectD& RectView) //коорд прямоугольника, охват проекцию пирамиды на XY ВСК
{
...
{
	// Рисует пирамиду, освещенную источнием заданного цвета 
	// Самостоятельный пересчет координат из мировых в оконные (MM_TEXT)
	// dc - ссылка на класс CDC MFC
	...
	CMatrix ViewVert = MV * Vertices; // Координаты вершин
	CRectD RectView;
	GetRect(ViewVert, RectView); //прямоугольник, охват проекцию пирамиды на XY ВСК
	...
	for (int i = 0; i < 3; i++) //рисования боковых граней пирамиды.
	{
		...
		CMatrix V1 = R2 - R1;          // Вектор – ребро в основании
		CMatrix V2 = VE - R1;          // Вектор – ребро к вершине
		VN = VectorMult(V2, V1);       //вектора нормали к грани пирамиды
		sm = cosv1v2(VN, ViewCart);	   // cos (N к грани, вект направления точки наблюденияы)
...
//косинус угла между двумя векторами ( вектором нормали к грани и вектором к точке наблюдения).
//Этот косинус используется для степени освещения (цвета) грани.
double cosv1v2(CMatrix& V1, CMatrix& V2)
{

	double modV1 = ModVec(V1);// Вычисляет модуль вектора V
