...
//содержит классы для создания главного окна приложения (CMyMainWnd) и приложения (CMyApp).
//В главном окне создается меню, и есть функции Matrix и Functions для выполнения различных операций с матрицами и вывода результатов на экран.
...
public:
	CMyMainWnd()
	{
		// Создание главного окна приложения с заголовком "Lab 1 "CMatrix""
		Create(NULL, L"Lab 1 \"CMatrix\"");
		
	}
	CMenu* menu;  // Указатель на меню приложения
	afx_msg int OnCreate(LPCREATESTRUCT);
	void Matrix();  // Метод для работы с матрицами
	void Functions();  // Метод для работы с функциями
	DECLARE_MESSAGE_MAP();
};
class CMyApp : public CWinApp
{
	public:
		...
		{
			// Создание объекта главного окна и отображение его
...
// Определение таблицы сообщений для главного окна
BEGIN_MESSAGE_MAP(CMyMainWnd, CFrameWnd)
	ON_WM_CREATE()  // Обработка сообщения WM_CREATE
	ON_COMMAND(ID_M, Matrix)  // Обработка команды ID_M (Matrix)
	ON_COMMAND(ID_F, Functions)  // Обработка команды ID_F (Functions)
END_MESSAGE_MAP()
...
afx_msg int CMyMainWnd::OnCreate(LPCREATESTRUCT)
{
	...
	SetMenu(menu);// Установка главного меню для окна
	return 0;
}
