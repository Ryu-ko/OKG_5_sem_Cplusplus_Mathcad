...
void CPlot3D::CreateMatrWindow()// Заполняет матрицу MatrWindow оконными координатами точки изображения
{
...
// Index=1 –  Зеркальная модель отражения света
{
	...
	CMatrix VSphere(3), VSphereNorm(3), V0(3), PV(4); //VSphere точки сферы, нормаль, начальная точка
	COLORREF Col;
	double df = 0.54;                              // Шаг по фи 
	double dq = 0.54;                              // Шаг угл тетта для расчета освещенности
	double kLight;
	VSphere(0) = 3;								   // радиус сферы
	...
	for (double fi = 0; fi < 360.0; fi += df)		  // фи 
		for (double q = 0; q <= 180.0; q += dq)		  // тетта
		{
			VSphere(1) = fi;
			VSphere(2) = q;
			...
			if (cos_RN > 0) // Если точка видима наблюдателю
			{
				...
				if (cos_PN > 0) // Если точка сферы освещается
				{
					if (Index == 0)				///диффуз
						kLight = cos_PN;
					else if (Index == 1)		///зеркал нормаль и свет
