...
void CPyramid::Draw1(CDC& dc,CMatrix& PView,CRect& RW)
{
	...
	CBrush Brus(RGB(200, 155, 126));			// Для нижнего основания
	...
	for(int i = 0; i < 3; i++)					// рисуем ребра без невидимых
	{
		...
		CMatrix V1 = R2 - R1;//первое ребро пирамиды
		CMatrix V2 = VE - R1;//второе ребро	пирамиды 
		VN = VectorMult(V2, V1);//получение нормаль

		sm = ScalarMult(VN, ViewCart);//определение, повёрнута нормаль к или от точки наблюдения
		
		if (sm >= 0)//если косинус неотрицателен, то угол между векторами <= 90град нормаль в точку наблюдения = ребро видимое
		{
			dc.MoveTo(MasVert[i]);			
			dc.LineTo(MasVert[k]);
