...
// fi - угол в градусах
{
	double fg = fmod(fi, 360.0); // остаток от деления х/у
...
void SetMyMode(CDC& dc, CRectD& RS, CRect& RW) // для анизотропии - различие совйств среды
...
// RW -	 Область в оконных координатах - int  
{
	...
	dc.SetMapMode(MM_ANISOTROPIC); // позволяет различным осям иметь разные масштабы
	dc.SetWindowExt((int)dsx, (int)dsy);
	dc.SetViewportExt(dwx, -dwy); // высота оконной области отрицательна, чтобы инвертировать направление оси Y
...
CBlade::CBlade()            
{
	...
	fiFT = 35; //угoл положение для FT
	wPoint = -8; //скорость
	dt = 0.1; //время между кадрами
...
void CBlade::GetRS(CRectD& RSX) { //Возвращает область графика в мировой СК
