...
//полином лагранж как он задается его вид чем отличается от предше
// задается Ln(x)=summa[n,i=0] yi ((x-x0)(x-x1)...(x-xn)) / ((xi-x0)(xi-x1)...(xi-xn))

// Кривая Безье — это математическая концепция, используемая в графике и компьютерной графике для создания плавных и красивых кривых.
// парам: система x=Px(t) y=Py(t) Значение t параметр, которому отвечают координаты отдельной точки линии
// Многочлены Безье:
// Px(t) = summa[m,i=0](Cm^i * t^i (1-t)^(m-i) * xi)
// Py(t) = summa[m,i=0](Cm^i * t^i (1-t)^(m-i) * yi)
// 
// геом алгоритм:
//каждая сторона контура многоугольника, проходящего по точкам - ориентирам, делится пропорционально значению t

//точки деления соединяются отрезками прямых и образуют новый многоугольник.Количество узлов нового контура на единицу 
// меньше, чем количество узлов предыдущего контура;

// стороны нового контура снова делятся пропорционально значению t. И так далее.

// Это продолжается до тех пор, пока не будет получена единственная точка деле ния.Эта точка и будет точкой кривой Безье(рис. 14.5а, б
...
// NT - число отрезков по параметру t
{
	...
	double dt = 1.0 / NT; //Вычисление шага параметра t для разбиения кривой на NT отрезко

	int N = X.rows();// Получение количества опорных точек
	CMatrix RX(N), RY(N);
	xs = X(0);   ys = Y(0);
	GetWindowCoords(xs, ys, xw, yw);
	//текущая позиция для рисования
	dc.MoveTo(xw, yw);

	for (int k = 1; k <= NT; k++) //по шагам параметра t
	{
		double t = k * dt; // значения t для текущего отрезка
		// Копирование новых координат опорных точек в RX и RY
		for (int i = 0; i < N; i++)
		{
			RX(i) = X(i);
			RY(i) = Y(i);
		}
		// Формула кривой Безье B(t)=summa(Pi*(Bi)^n(t))
		for (int j = N - 1; j > 0; j--) //итерирует по отрезкам кривой
		{
			for (int i = 0; i < j; i++) //итерирует по опорным точкам для вычисления промежуточных результатов
			{
				RX(i) = RX(i) + t * (RX(i + 1) - RX(i)); // Px(t) = summa[m,i=0](Cm^i * t^i (1-t)^(m-i) * xi)
				RY(i) = RY(i) + t * (RY(i + 1) - RY(i)); // Py(t) = summa[m,i=0](Cm^i * t^i (1-t)^(m-i) * yi)
			}
		}
		// Получение новых координат текущей точки и отрисовка линии
		xs = RX(0);   ys = RY(0);
		GetWindowCoords(xs, ys, xw, yw);
		dc.LineTo(xw, yw);
	}
...
// L(x)=summa(yi*multiple((x-xj)/(xi-xj)))
double Lagr(CMatrix& X, CMatrix& Y, double x, int size)
{
	double lagrange_pol = 0; //текущее значение многочлена Лагранжа
	double basics_pol;

	for (int i = 0; i < size; i++)//перебирает точки данных
	{
		basics_pol = 1;
		for (int j = 0; j < size; j++) //вычисл базовый полином
		{
			if (j == i)
				continue;
...
void CPlot2D::DrawLagr(CDC& dc)
{
	...
	double xL = 0;
	double xH = pi;
	int N = (xH - xL) / dx; //количество опорных точек
	dx = 0.1;
	int NL = (xH - xL) / dx;//новое количество опорных точек
	CMatrix XL(NL + 1);
	CMatrix YL(NL + 1);

	for (int i = 0; i <= NL; i++) //Заполнение матрицы координат
	{
		XL(i) = xL + i * dx;
